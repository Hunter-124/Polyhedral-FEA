...
Declares what to sweep. Written by hand, by the GUI, or by a generator
script. Real production example: `bench/campaigns/advisor-truth-0/campaign.json`
(generated by `scripts/gen_primitive_corpus.py`).

```jsonc
{
  "name": "advisor-truth-0",
  "comment": "…",                  // optional free-text provenance note
  "warehouse": true,               // write the full run warehouse (§7)
  "parts": ["tests/fixtures/parts/plate_hole.case.json"],  // case files, §4
  "tiers": [                       // fidelity ladder for branch trimming
    { "h_scale": 1.0, "keep_frac": 1.0 }
  ],
  "grid": {                        // full factorial at tier 0 (no stone unturned)
    "mesher":            ["graded_tet"],
    "h_rel":             [0.06, 0.038, 0.024],  // h as a fraction of the part's
                                                // bbox diagonal; resolved to an
                                                // absolute h per run (§3 `h`)
    "order":             [2],          // grows as p-hierarchical lands
    "adapt_passes":      [3],
    "eta_target":        [0.01],
    "element_tendency":  [0.0],        // ∈[-1,+1]; see resolve_element_tendency
    "feature_refine":    [true],
    "bc_grading":        [true],
    "skin_layers":       [2],
    "p_elevate":         [false],
  ...
  "score": {                       // Pareto axes; scalar score for trimming
    "weights": { "accuracy": 1.0, "solve_ms": 0.0, "mesh_ms": 0.0 }
  },
  "resources": {
    "max_threads": 0,               // 0 = unlimited
    "max_mem_gb": 0,                // 0 = unlimited
    // M14 wall-clock kills (ADR-0024 Q9):
    "max_run_wall_s": 1800,         // 0 → tier defaults: tier0=900, tier1=900, tier2+=2700
    "max_pack_wall_s": 172800       // 0 = unlimited pack ceiling; else stop *starting* new runs
  }
}
```
...
`keep_frac` advance. `accuracy` per metric is the relative error against the
case's hand-calc truth (§5), mapped to a 0–1 score as `1/(1+|rel_err|/tol)`
with `tol` from the reference metric, and zeroed when health fails; every
metric's value/truth/rel_err/score lands in `accuracy.all` on the results
row (§3).
...
Successful row (`status: "ok"`):

```jsonc
{
  "schema": "advisor-row-v3",       // row format version
  "cfg_id": "cfg-97015a68",         // stable hash-id of the config values
  "config": {                       // the swept grid values (§1), verbatim
    "mesher": "graded_tet", "h_rel": 0.06, "order": 2,
    "adapt_passes": 3, "eta_target": 0.01, "element_tendency": 0.0,
    "feature_refine": true, "bc_grading": true, "skin_layers": 2,
    "p_elevate": false, "adapt_leb_waves": 2
  },
  "action": { "...": "..." },       // config + resolved "h" (absolute size, m)
  "part": "box_hole_s0_c0", "tier": 0,
  "h": 0.004008,                    // resolved absolute element size (m)
  "mesher_note": "graded tet v6 (…): 13098 tets …",  // mesher self-report
  "geom_class": {
  ...
  "features": {                   // advisor feature vector (geometry + BCs +
    "volume": 0, "surface_area": 0, "sa_over_v23": 0,   // loads); full key set:
    "bbox_dx": 0, "bbox_dy": 0, "bbox_dz": 0, "diag": 0, // volume, surface_area,
    "n_faces": 0, "curved_frac": 0, "min_feature_h": 0,  // sa_over_v23, bbox_d{x,y,z},
    "kappa_max_h": 0, "kappa_mean_h": 0,                 // diag, n_faces, curved_frac,
    "n_sharp_edges": 0, "sharp_edge_len_total": 0,       // min_feature_h, kappa_max_h,
    "thin_min_over_diag": 0, "thin_p10_over_diag": 0,    // kappa_mean_h, n_sharp_edges,
    "n_load_faces": 0, "load_area_frac": 0,              // sharp_edge_len_total,
    "n_fix_faces": 0, "fix_area_frac": 0,                // thin_{min,p10}_over_diag,
    "load_dir_x": 0, "load_dir_y": 0, "load_dir_z": 0,   // n_{load,fix}_faces,
    "load_axis_alignment": 0, "fix_load_dist_over_diag": 0, // {load,fix}_area_frac,
    "poisson": 0                    // load_dir_{x,y,z}, load_axis_alignment,
  },                                // fix_load_dist_over_diag, poisson
  "geometry_volume_err": 5.2e-5,    // mesh vs geometry volume rel err
  "geometry_fill_volume_err": 2.6e-4,
  "geo_fidelity": {                 // surface-sampling fidelity vs CAD
    "available": true, "n_samples": 3756,
    "dist_max": 0.0114, "dist_p95": 2.6e-17, "dist_p99": 0.0016,
    "chamfer_mean": 3.7e-5, "normal_angle_p95_rad": 0.0, "rel_volume_err": 0.0
  },
  ...
  "answers": {
    ...
    "sigma_box_max": 9.12e7,        // max VM inside the metric's select box
    ...
    "mean_u_component": 1.2e-4,     // face-mean signed dominant-axis u
    "mean_ux": 1.2e-4, "mean_uz": 0.0,  // per-axis face-mean components
    "dominant_load_axis": 0,
    // --- load-face selection audit ---
    "load_face_area": 0.00785,      // selected load face area (m²)
    "mesh_selected_area": 0.00785,
    "load_area_rel_err": 0.01,      // |A_sel − A_expected| / A_expected when expected_area set
    "load_area_status": "rescaled_to_exact_cad",  // verified | rescaled_to_exact_cad | unverified
    "authored_area_checked": true,  // case expected_area cross-checked vs CAD-rule area
    "authored_area_consistent": true,
    "authored_area_rel_diff": 3.6e-11,
    "n_probe_nodes": 48,
    "n_load_faces": 12
  },
  "health": {
    ...
    "load_area_rel_err": 0.01,
    "load_area_status": "rescaled_to_exact_cad",
    "load_area_verified": true,
    "authored_area_consistent": true,
  ...
  "scorecard": {
    ...
    "n_dof": 35805,
    "accuracy_rel_err": 0.115,      // first metric rel_err (case-specific probe)
    "min_element_quality": 0.17,    // quality.M6 when present
    "solve_residual_rel": 1.2e-12,  // free residual (gate, not score)
    "health_ok": true
  },
  "accuracy": {                     // primary metric (first of all), duplicated flat…
    "metric": "scf", "value": 2.763, "truth": 3.124,
    "rel_err": 0.115, "score": 0.368, "trusted": true,
    "all": [                        // …and every metric scored against its truth (§5):
      { "metric": "scf", "value": 2.763, "truth": 3.124,
        "rel_err": 0.115, "score": 0.368, "trusted": true }
      // score = 1/(1+|rel_err|/tol), tol from the reference metric;
      // trusted = health.ok (false → score zeroed, values still recorded)
    ]
  },
...
Failed rows keep the identifying fields (`schema`, `cfg_id`, `config`,
`action`, `part`, `tier`, `h`, `features`, `geom_class`, timings,
`n_pred_elems`, `mesher_note`, `status`) and add:

```jsonc
{ "error": "adaptive SolveJob failed: solve failed: solve_elastostatics: CG failed (…)" }
```

`status: "over_budget"` rows additionally carry `over_budget_cause`
(`"sizing"` | `"mesher"` | `"budget"` | `"wall_clock"` — sizing: N_pred ≫
tier; mesher: N_pred OK but N_actual ≫ N_pred; budget: hard-kill ≈2×
DOF/elem; wall_clock: M14 per-run wall exceeded). `answers` / `health` /
`scorecard` / `accuracy` are absent on non-ok rows — analyze must treat
their presence as `status == "ok"`.
