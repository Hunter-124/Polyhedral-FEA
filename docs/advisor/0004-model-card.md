...
**Currency, 2026-08-17.** The shipped artifact is the v5 model:
`bench/advisor/dataset.csv`, **2,672 rows**, sha256 `d4367ab0b7456ed6…`,
`runs/best.pt` run 58, ONNX 62 input columns / 17,617 parameters, parity
2.197e-06. The corpus was re-measured from the STEP files after
[ADR-0035](../decisions/0035-boundary-conformity.md) moved every boundary node
onto the exact BRep. References are an independent Gmsh + CalculiX chain plus,
...
`bench/advisor/crossval_v5.json` and are read in
[0009](0009-v5-mesher-conformity-retrain.md), which also records the honest
result: predictions improved, decision regret did not.
...
[0009](0009-v5-mesher-conformity-retrain.md) (v5 corpus, current decision
numbers).
