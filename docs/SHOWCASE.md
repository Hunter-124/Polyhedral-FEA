...
- **Advisor cinema** (`assets/cinema/advisor_cinema.mp4`, `.gif`, `poster.png`)
  ...
  positive-volume solid. Two chassis-bushing outer sleeves form separate fixed
  supports; two swept non-coplanar arms and a curved brace converge on the
  loaded upright-side boss. The conserved proof-load resultant combines vertical
  and inboard components, producing bending and torsion rather than token axial stress.

  The recorded result has 40,170 tet4 cells, 9,796 nodes and 29,388 unknowns;
  minimum/mean shape quality is 0.0200 / 0.2496. The 47.17 kN distributed proof
  load produces 33.16 MPa true peak von Mises stress (17.42 MPa p99) and
