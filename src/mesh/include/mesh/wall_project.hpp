...
/// project displacement into the local tangent plane (OCC normal), then
/// re-project through the persistent owner-aware oracle when supplied. A known
/// face/edge/vertex owner is never replaced by a global surface projection.
///
...
/// or there are no free-boundary wall nodes. Callers without retained
/// provenance may omit `projection` and use the live-BRep global fallback.
[[nodiscard]] WallProjectStats
wall_tangential_project(const geom::CadModel& cad, const geom::CadTopology* topo,
                        std::vector<Eigen::Vector3d>& nodes,
                        std::span<const std::array<std::uint32_t, 4>> tets, double h,
                        int iters = 4, double relax = 0.45, double sharp_guard_frac = 0.4,
